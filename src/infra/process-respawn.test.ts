...
function makeSpawnedChild(pid: number) {
  const child = {
    pid,
    on: vi.fn(),
    unref: vi.fn(),
  ...
  child.on.mockImplementation((_event: string, _listener: (err: Error) => void) => child);
  return child;
}
...
describe("respawnGatewayProcessForUpdate", () => {
  ...
  it("allows detached respawn on unmanaged Windows during updates", () => {
    ...
    spawnMock.mockReturnValue(makeSpawnedChild(5151));
  ...
  it("rewrites a pnpm-versioned OpenClaw entry before detached update respawn", () => {
    ...
    spawnMock.mockReturnValue(makeSpawnedChild(7171));
  ...
  it("does not rewrite another package's pnpm-versioned entry", () => {
    ...
    spawnMock.mockReturnValue(makeSpawnedChild(8181));
  ...
  it("spawns a detached update process when macOS only has inherited XPC state", () => {
    ...
    process.execArgv = [];
    process.argv = ["/usr/local/bin/node", "/repo/dist/index.js", "gateway", "run"];
    spawnMock.mockReturnValue(makeSpawnedChild(6161));
  ...
  it("attaches an error listener before unrefing the detached update process", () => {
    clearSupervisorHints();
    setPlatform("linux");
    process.execArgv = [];
    process.argv = ["/usr/local/bin/node", "/repo/dist/index.js", "gateway", "run"];
    const child = makeSpawnedChild(9191);
    spawnMock.mockReturnValue(child);

    const result = respawnGatewayProcessForUpdate();

    expect(result).toMatchObject({ mode: "spawned", pid: 9191 });
    expect(child.on).toHaveBeenCalledWith("error", expect.any(Function));
    expect(child.on.mock.invocationCallOrder[0]).toBeLessThan(
      child.unref.mock.invocationCallOrder[0] ?? Number.POSITIVE_INFINITY,
    );
