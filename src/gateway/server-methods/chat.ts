...
async function appendAssistantTranscriptMessage(params: {
  ...
  if (params.idempotencyKey && params.agentId) {
              ...
              if (!agentRunStarted) {
                ...
                if (sourceReplyPayloads.length > 0) {
                  ...
                  const { databasePath: latestDatabasePath, entry: latestEntry } =
                    loadSessionEntry(sessionKey);
                  const sessionId = latestEntry?.sessionId ?? backingSessionId ?? clientRunId;
                  const transcriptScope: SourceReplyTranscriptScope = {
                    agentId,
                    path: latestDatabasePath,
                    sessionId,
                  };
                  const mediaLocalRoots = appendLocalMediaParentRoots(
                    getAgentScopedMediaLocalRoots(cfg, agentId),
                    latestDatabasePath ? [latestDatabasePath] : undefined,
                  );
                  ...
                  for (const [replyIndex] of sourceReplyPayloads.entries()) {
                    ...
                    const replyBroadcastContent = hasAssistantDisplayMediaContent(
                      ...
                      : hasAssistantDisplayMediaContent(replyMediaMessage?.content)
                        ...
                        : replyAssistantContent;
                    const persistedContent = hasAssistantDisplayMediaContent(replyAssistantContent)
                      ...
                      : hasAssistantDisplayMediaContent(replyMediaMessage?.content)
                        ? replaceAssistantContentTextBlocks(
                            replyMediaMessage?.content as AssistantDisplayContentBlock[],
                            replyAssistantContent ? { content: replyAssistantContent } : null,
                          )
                        : replaceAssistantContentTextBlocks(
                            replyAssistantContent,
                            replyMediaMessage ?? null,
                  ...
                  if (sourceReplyBroadcastContent.length || displayReply) {
                    ...
                    if (sourceReplyPersistenceRequests.length > 0) {
                      ...
                      if (rewriteTargets.length > 0) {
                        ...
                        const canRewriteMirrors = await canRewriteSourceReplyMirrors({
                          ...transcriptScope,
                          allowedSourceReplyMirrorIds,
                          rewriteTargetIds,
                        });
                        if (canRewriteMirrors) {
                          ...
                          for (const target of rewriteTargets) {
                            target.request.state.backedManagedOutgoingContent = true;
                          }
                          if (result.changed) {
                            for (const target of rewriteTargets) {
                              const rewritten =
                              ...
                              await attachSourceReplyManagedImages({
                                messageId: rewritten?.messageId ?? target.messageId,
