...
export async function runCliTurnCompactionLifecycle(params: {
  ...
  if (!sessionEntry?.sessionId || !contextTokenBudget) {
    return sessionEntry;
  }
  if (
    ...
    // Explicit /compact or plugin compaction still forwards through the harness path.
  ...
  const tokenSnapshot = resolveSessionTokenSnapshot(sessionEntry);
  ...
  if (isNativeHarnessCompactionSession(sessionEntry, params.provider)) {
