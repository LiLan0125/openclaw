...
const resolveSessionRowLookup = (
  ...
  try {
    ...
    return {
      sessionKey,
      entries,
      entry: resolveSessionRowEntry({ entries, sessionKey }).existing,
    };
  } catch {
    return {
      sessionKey,
...
export async function dispatchReplyFromConfig(
  ...
  // initialSessionRowEntry is command-target-aware, so native command turns
  ...
  const harnessDefaultVisibleReplies =
    configuredVisibleReplies === undefined && chatType !== "group" && chatType !== "channel"
      ? resolveHarnessSourceVisibleRepliesDefault({
          cfg,
          ctx,
          entries: sessionRowEntry.entries,
          entry: sessionRowEntry.entry,
          sessionAgentId,
          sessionKey: acpDispatchSessionKey,
