...
describe("dispatchReplyFromConfig ACP abort", () => {
  ...
  it("keys bound ACP tail abort ownership to the source dispatch session", async () => {
    ...
    for (const [key, entry] of Object.entries(sessionEntries)) {
      sessionStoreMocks.entries.set(key, entry);
    }
    sessionStoreMocks.resolveSessionRowEntry.mockImplementation((...args: unknown[]) => {
    ...
    acpMocks.readAcpSessionEntry.mockImplementation((params: { sessionKey: string }) =>
      params.sessionKey === boundAcpSessionKey
        ? {
            ...
            cfg: {},
            storePath: "/tmp/mock-sessions.json",
            entry: sessionEntries[boundAcpSessionKey],
            acp: {
              backend: "acpx",
              agent: "codex",
