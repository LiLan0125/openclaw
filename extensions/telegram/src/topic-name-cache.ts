...
function openTopicNamePersistentStore(namespace: string): TopicNamePersistentStore {
  const testStore = topicNameStoreFactoryForTest?.(namespace);
  if (testStore) {
    return testStore;
  }
  const store = createPluginStateSyncKeyedStore<TopicEntry>("telegram", {
    namespace,
...
function openTopicNameSyncStore(namespace: string, env?: NodeJS.ProcessEnv) {
  return createPluginStateSyncKeyedStore<TopicEntry>("telegram", {
    namespace,
    maxEntries: TELEGRAM_TOPIC_NAME_CACHE_MAX_ENTRIES,
    ...(env ? { env } : {}),
  });
...
export function importTelegramTopicNameEntry(
  ...
  const stateKey = scope ?? DEFAULT_TOPIC_NAME_CACHE_SCOPE;
  const key = cacheKey(chatId, threadId);
  openTopicNameSyncStore(namespaceForScope(stateKey), options?.env).register(key, entry);
  if (!options?.env) {
    const state = getTopicStoreState(stateKey);
    state.store.set(key, entry);
    state.lastUpdatedAt = Math.max(state.lastUpdatedAt, entry.updatedAt);
    state.hydrated = true;
  }
}
